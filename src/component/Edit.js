...
function Edit() {
  ...
  useEffect(() => {
    axios.get(`https://port-0-backend2025-mbeeobco2e6ef2af.sel4.cloudtype.app/mypage/item/${id}`)
      .then(res => {
        const data = res.data;
        setForm({
          title: data.title || '',
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      await axios.post(`https://port-0-backend2025-mbeeobco2e6ef2af.sel4.cloudtype.app/mypage/${id}`, { ...form, img: imgValue });
      alert('수정이 완료되었습니다.');
      navigate('/mypage');
    } catch (err) {
      console.error(err);
