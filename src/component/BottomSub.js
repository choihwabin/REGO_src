...
const BottomSub=({setCartCount}) => {
  ...
  const addToCart = async () => {
    ...
    try{
      const res = await fetch('https://port-0-backend2025-mbeeobco2e6ef2af.sel4.cloudtype.app/cart', {
        method:'POST',
    ...
    if(res.ok){
      setIsModalOpen(true);
      try{
      const updatedRes = await fetch(`https://port-0-backend2025-mbeeobco2e6ef2af.sel4.cloudtype.app/cart?userId=${userId}`);
